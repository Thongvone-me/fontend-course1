...
const ProductCard: React.FC<Props> = ({item}) => {
    ...
    const handleAddCard = (evt: React.MouseEvent, item_: any) => {
        evt.stopPropagation();
        const obj = {
  ...
  return (
   <React.Fragment>
        ...
        actions={[
        <Button icon={<FaPlus />} type='link' onClick={(e)=>handleAddCard(e, item)}>Add cart</Button>,
    ...
    >
        <Meta
            ...
            title={<Typography.Title level={3} className=' line-clamp-1 !m-0 !p-0'>{item?.name ?? ""}</Typography.Title>}
            description={<Content>
                <Typography.Text className=' line-clamp-3 !m-0 !p-0'>{item?.slug ?? item?.description ?? "...."}</Typography.Text>
                <Typography.Title level={3}>{`${item.sellPrice} ₭`}</Typography.Title>
            </Content>}
        />
    </Card>
    <Modal
